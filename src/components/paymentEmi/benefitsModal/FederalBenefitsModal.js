...
export default class FederalBenefitsModal extends Component {
  render() {
    const { lenderLogo } = lenderTheme['102'];
    return (
      <div>
        <TopAppBar
          class={`${style.lenderBenefits} ${style.federalTheme}`}
          style={'height: 100% !important'}>
          <div>
            <span>
              <img align='left' class={`${style.lenderImg}`} src={lenderLogo} />
              <img
                align='right'
                id='dismissBenefits'
          ...
          <div className={`${style.lenderBenefitsTitle}`}>
            You are eligible to complete this purchase with EMI on your Federal
            Bank Cardless EMI
          </div>
